...
const SETTINGS_KEY = 'polihov-products-github-v3';
...
function persistGithubSettingsFromForm() {
  githubSettings = {
    ...
    path: el.ghPath.value.trim() || 'data/items.json',
    token: el.ghToken.value.trim()
  };
  localStorage.setItem(SETTINGS_KEY, JSON.stringify(githubSettings));
  setSyncMessage('Настройки GitHub сохранены на этом устройстве.', 'helper');
}

function applyGithubSettingsToForm() {
  el.ghOwner.value = githubSettings.owner || '';
  el.ghRepo.value = githubSettings.repo || '';
  el.ghBranch.value = githubSettings.branch || 'main';
  el.ghPath.value = githubSettings.path || 'data/items.json';
  ...
  state.lists.forEach((list) => {
    ...
    node.innerHTML = `
      <div class="list-item-head">
        ...
        <div class="dropdown">
          <button class="menu-trigger" aria-label="Меню списка" data-dropdown-toggle="${menuId}">
          ...
          <div id="${menuId}" class="dropdown-menu glass hidden" data-align="left" role="menu">
            <button class="dropdown-item" data-action="edit" role="menuitem">
            ...
            <button class="dropdown-item is-danger" data-action="remove" role="menuitem">
  ...
  items.forEach((item) => {
    ...
    card.innerHTML = `
      <div class="card-menu-wrap dropdown">
        <button class="menu-trigger" aria-label="Меню товара" data-dropdown-toggle="${menuId}">
          <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 5h.01M12 12h.01M12 19h.01"/></svg>
        </button>
        <div id="${menuId}" class="dropdown-menu glass hidden" role="menu">
          ...
          <button class="dropdown-item is-danger" data-action="delete" role="menuitem">
            <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 6h18M8 6V4h8v2m-9 0 1 14h8l1-14"/></svg>
      ...
      <div class="card-inner">
        ...
        <div class="card-body">
          ...
          <div class="inline-actions">
            ${item.buyUrl ? `<a class="buy-link" href="${escapeAttribute(item.buyUrl)}" target="_blank" rel="noopener noreferrer"><button class="btn-small btn-primary" type="button"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M14 3h7v7M10 14 21 3M21 14v5a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5"/></svg>Открыть товар</button></a>` : ''}
          </div>
        </div>
      </div>
    `;
...
function validateGithubSettings() {
  ...
  if (!githubSettings.owner || !githubSettings.repo || !githubSettings.branch || !githubSettings.path || !githubSettings.token) {
    throw new Error('Заполни owner, repo, branch, путь к JSON и token.');
  }
}
...
async function saveToGithub() {
  try {
    ...
    const base = `https://api.github.com/repos/${githubSettings.owner}/${githubSettings.repo}/contents/${encodeURIComponent(githubSettings.path).replace(/%2F/g, '/')}`;
    let sha;
    try {
      const existing = await githubRequest(`${base}?ref=${encodeURIComponent(githubSettings.branch)}`);
      sha = existing.sha;
    } catch (error) {
...
async function loadFromGithub() {
  try {
    ...
    const base = `https://api.github.com/repos/${githubSettings.owner}/${githubSettings.repo}/contents/${encodeURIComponent(githubSettings.path).replace(/%2F/g, '/')}`;
    const file = await githubRequest(`${base}?ref=${encodeURIComponent(githubSettings.branch)}`);
